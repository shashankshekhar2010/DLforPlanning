...
{	
	...
	public void callForDatasetGeneration() 
	{
		...
		try {
			...
			file_words = new File("/home/shashank/Dropbox/Bgu-Files/bgu.dl.heuristic/eclipse/bgu.learning/src/bgu/dl/features/Dictionary.txt");
			writer_words = new BufferedWriter(new FileWriter(file_words));

			// words (for word-2-vec representation)
			file_context = new File("/home/shashank/Dropbox/Bgu-Files/bgu.dl.heuristic/eclipse/bgu.learning/src/bgu/dl/features/ContextOfEachWordInTheDict.txt");
			...
			while (counter++ < 10 && listParentChildSuccessors.size() > 0) 
			{		
				...
				ArrayList<ArrayList<ArrayList>> getParentChildSuccessors = getParentChildSuccessors(parentChildStates, writer, writer_context, listOfPossiblePropositions);
	...
	private ArrayList<ArrayList<ArrayList>> getParentChildSuccessors(ArrayList<ArrayList> parentChildStates, Writer writer, Writer writer_context, ArrayList<AtomicFormula> listOfPossiblePropositions_1)
	{
		...
		while(target >=0 )
		{
			...
			// Function call for finding the context, that returns nothing
			callForTheContextOfTheLiteralsInTheCurrentState(listOfIntegersCorrespondingToLiterals, writer_context);	
